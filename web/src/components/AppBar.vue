<template>
  <v-app-bar app dense color="white" elevation="1">

    
    <v-toolbar-items v-if="isAuthenticated">
      <v-btn text to="/">
        <v-icon :left="!collapse" color="primary">mdi-home</v-icon>
      ...
      <v-btn
        ...
        text
      >
        <v-icon :left="!collapse" color="secondary">{{ item.icon }}</v-icon>
        <span v-if="!collapse">{{ item.text }}</span>
      </v-btn>
      <v-btn text @click="logout()">
        <v-icon :left="!collapse" color="primary">mdi-exit-to-app</v-icon>
...
export default class AppBar extends Vue {
  ...
  menu = [
    // {text: "Profile", to: "/profile", icon:"mdi-upload"},
  ]
